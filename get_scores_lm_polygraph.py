...
def time_calculator(func: Callable) -> Callable:
    """Decorator to measure execution time of calculator methods."""

    @functools.wraps(func)
    def wrapper(
        self, deps: Dict[str, Any], texts: List[str], model: Any
    ) -> Dict[str, Any]:
...
class VLLMModel:
    ...
    def _load_model(self):
        ...
        # Initialize vLLM
        self.llm = LLM(
            model=self.model_name, gpu_memory_utilization=self.gpu_memory_utilization
        )
        ...
        self.model = WhiteboxModelvLLM(
            self.llm, self.sampling_params, device=self.device
        )

        tqdm.write(f"vLLM model loaded: {self.model_name}")

    def prepare_chat_messages(
        self, question: str, doc_text: str = None
    ) -> List[Dict[str, str]]:
...
Question: {question}"""
    ...
    def generate(
        ...
        return {"text": generated_text, "tokens": generated_tokens}
...
class PolygraphCalculator:
    ...
    def initialize_estimators(self):
        ...
        self.calc_nli = GreedyAlternativesNLICalculator(
            nli_model=self.nli_model, batch_size=300
        )
        self.calc_samples = SamplingGenerationCalculator()
        self.calc_cross_encoder = CrossEncoderSimilarityMatrixCalculator(
            device="cuda:3", batch_size=100
        )
        self.calc_semantic_matrix = SemanticMatrixCalculator(
            nli_model=self.nli_model, batch_size=2
        )
    ...
    def calculate_uncertainty_batch(self, prompts: List[str]) -> List[Dict[str, float]]:
        ...
        for i in tqdm(
            ...
            desc="Processing scenarios in texts",
            total=len(prompts) // self.batch_size,
            leave=False,
        ):
            ...
            calculators = [
                (
                    "calc_infer_llm",
                    lambda: self.calc_infer_llm(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_nli",
                    lambda: self.calc_nli(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_samples",
                    lambda: self.calc_samples(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_cross_encoder",
                    lambda: self.calc_cross_encoder(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_semantic_matrix",
                    lambda: self.calc_semantic_matrix(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_semantic_classes",
                    lambda: self.calc_semantic_classes(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_entropy",
                    lambda: self.calc_entropy(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
                (
                    "calc_prompt",
                    lambda: self.calc_prompt(
                        deps, texts=batch, model=self.vllm_model.model
                    ),
                ),
            ]
    ...
    def calculate_uncertainty_all_scenarios(
        self, question: str, doc_text: str, individual_doc_texts: List[str]
        ...
        naive_prompt = self.vllm_model.tokenizer.apply_chat_template(
            naive_messages, tokenize=False
        )
        ...
        rag_prompt = self.vllm_model.tokenizer.apply_chat_template(
            rag_messages, tokenize=False
        )
        messages.append(rag_prompt)

        # Individual document scenarios
        for individual_doc_text in individual_doc_texts:
            individual_messages = self.vllm_model.prepare_chat_messages(
                question, individual_doc_text
            )
            individual_prompt = self.vllm_model.tokenizer.apply_chat_template(
                individual_messages, tokenize=False
            )
        ...
        individual_docs_metrics = (
            all_uncertainties[2:] if len(all_uncertainties) > 2 else []
        )
...
class RAGEvaluator:
    ...
    def __init__(
        ...
        self.polygraph_calculator = PolygraphCalculator(
            self.vllm_model, batch_size=self.batch_size
        )
    ...
    def evaluate_question(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        naive_metrics, rag_metrics, individual_docs_metrics = (
            self.polygraph_calculator.calculate_uncertainty_all_scenarios(
                question=question,
        ...
        for metric_name in naive_metrics:
            if metric_name in rag_metrics:
                ...
                if any(
                    ...
                    utility[reduction_name] = float(naive_metrics[metric_name]) - float(
                        rag_metrics[metric_name]
                    )
                else:
                    utility[reduction_name] = float(rag_metrics[metric_name]) - float(
                ...
                for metric_name in naive_metrics:
                    if metric_name in doc_metrics:
                        ...
                        if any(
                            ...
                            doc_utility[reduction_name] = float(
                                naive_metrics[metric_name]
                            ) - float(doc_metrics[metric_name])
                        else:
                            doc_utility[reduction_name] = float(
    ...
    def evaluate_dataset(
        self, dataset_path: str, output_path: str, max_samples: Optional[int] = None
    ) -> None:
        ...
        if os.path.exists(output_dir):
            ...
            temp_files = [
                ...
                if f.startswith(f"{base_output_name}_temp_") and f.endswith(".json")
            ]
            ...
            if temp_files:
                ...
                for temp_file in temp_files:
                    try:
                        ...
                        idx_str = temp_file.replace(
                            f"{base_output_name}_temp_", ""
                        ).replace(".json", "")
                ...
                if indices:
                    ...
                    tqdm.write(
                        f"Found existing temporary files. Resuming from item {latest_index + 1}"
                    )
        ...
        for i in range(start_index, len(data)):
            item = data[i]
            tqdm.write(
                f"Processing question {i + 1}/{len(data)}: {item['question'][:50]}..."
            )
            ...
            temp_output_path = os.path.join(
                output_dir, f"{base_output_name}_temp_{i + 1}.json"
            )
