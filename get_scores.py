...
from seper.calculate import calculate_uncertainty_soft_batch, create_collate_fn, process_item_for_seper
...
from seper.uncertainty_measures.semantic_entropy import get_semantic_ids, logsumexp_by_id, predictive_entropy_rao
...
class RerankerModel:
    ...
    def _load_model(self):
        ...
        self.model.eval()
        tqdm.write(f"Reranker model loaded: {self.model_name} (Type: {self.model_type})")
    ...
    def get_inputs_llm(self, pairs, prompt=None, max_length=1024):
        ...
        for query, passage in pairs:
            ...
            passage_inputs = self.tokenizer(
                f"B: {passage}", return_tensors=None, add_special_tokens=False, max_length=max_length, truncation=True
            )
...
class APIClient:
    ...
    def make_request(
        self, model_name: str, messages: List[Dict[str, str]], temperature: float, max_tokens: int, num_logprobs: int
    ) -> Dict[str, Any]:
...
class MetricsCalculator:
    ...
    def extract_logprobs(response_data: Dict[str, Any]) -> List[float]:
        ...
        try:
            tokens_data = response_data.get("choices", [{}])[0].get("logprobs", {}).get("content", [])
    ...
    def calculate_metrics(self, logprobs: List[float]) -> Dict[str, float]:
        ...
        return {"entropy": self.calculate_entropy(logprobs), "perplexity": self.calculate_perplexity(logprobs)}
    ...
    def prepare_chat_messages(question: str, doc_text: str = None) -> List[Dict[str, str]]:
        ...
        if doc_text:
            return [
                ...
                {"role": "user", "content": f"The following are given documents: {doc_text}\n\nQuestion: {question}"},
            ]
        else:
            return [
                {
...
class SePerCalculator:
    ...
    def prepare_input(
        self, question: str, answers: List[str], context: str, generations: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def calculate_seper(self, input_data: Dict[str, Any], computation_chunk_size: int = 8) -> float:
        ...
        with torch.no_grad():
            ...
            results = calculate_uncertainty_soft_batch(seper_input, self.entailment_model, computation_chunk_size)
...
class SemanticEntropyCalculator:
    ...
    def calculate_semantic_entropy(
        ...
        # 创建example格式
        example = {"question": question, "context": context, "reference": {"answers": {"text": answers}}}

        # tqdm.write(f"{question}\n {len(context.split('Doc'))-1}\n {answers}")

        # 计算语义ID (对回答进行语义聚类)
        semantic_ids = get_semantic_ids(
            responses, model=self.entailment_model, strict_entailment=strict_entailment, example=example
        )

        # 计算语义熵
        log_likelihood_per_semantic_id = logsumexp_by_id(semantic_ids, log_liks, agg="sum_normalized")
...
class RAGEvaluator:
    ...
    def __init__(
        ...
        if use_seper or use_semantic_entropy:
            ...
            self.shared_entailment_model = EntailmentDeberta(device=model_device)
        ...
        if use_seper:
            self.seper_calculator = SePerCalculator(device=model_device, entailment_model=self.shared_entailment_model)
        ...
        if use_semantic_entropy:
            self.semantic_entropy_calculator = SemanticEntropyCalculator(
                device=model_device, entailment_model=self.shared_entailment_model
            )
    ...
    def _get_response(self, question: str, doc_text: str = None) -> Dict[str, Any]:
        ...
        return {"response": response["choices"][0]["message"]["content"], "metrics": metrics, "logprobs": logprobs}

    def _generate_multiple_answers(self, question: str, context: str) -> List[Dict[str, Any]]:
        ...
        for _ in range(self.num_generations):
            ...
            if "choices" in response and len(response["choices"]) > 0:
                ...
                results.append({"text": generated_text, "logprob": avg_logprob})

        return results

    def _calculate_reranker_scores(
        self, question: str, retrieved_docs: List[Dict[str, Any]], batch_size: int = 8
    ) -> List[float]:
    ...
    def _process_individual_docs(
        self, question: str, naive_metrics: Dict[str, float], retrieved_docs: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_seper_metrics(
        ...
        try:
            ...
            if baseline_generations is None:
                baseline_generations = self._generate_multiple_answers(question, "")

            if all_docs_generations is None:
                all_docs_generations = self._generate_multiple_answers(question, doc_text)

            # 准备基线数据
            baseline_data = self.seper_calculator.prepare_input(
                question=question, answers=golden_answers, context="", generations=baseline_generations
            )

            # 提前计算基线SePer，只计算一次
            baseline_seper = self.seper_calculator.calculate_seper(
                input_data=baseline_data, computation_chunk_size=self.computation_chunk_size
            )

            # 准备所有文档数据
            all_docs_data = self.seper_calculator.prepare_input(
                question=question, answers=golden_answers, context=doc_text, generations=all_docs_generations
            )
            ...
            for i, doc in enumerate(retrieved_docs):
                ...
                if single_doc_generations is None:
                    single_doc_generations = self._generate_multiple_answers(question, single_doc_text)
    ...
    def evaluate_question(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        individual_doc_results = self._process_individual_docs(question, naive_result["metrics"], retrieved_docs)

        # Calculate reranker scores if enabled
        if self.use_reranker:
            reranker_scores = self._calculate_reranker_scores(question, retrieved_docs)
        ...
        if self.use_seper and self.use_semantic_entropy:
            tqdm.write("Generating shared samples for both SePer and Semantic Entropy...")
            baseline_generations = self._generate_multiple_answers(question, "")
            all_docs_generations = self._generate_multiple_answers(question, doc_text)
        ...
        if self.use_seper:
            ...
            if seper_results:
                ...
                if self.use_semantic_entropy and baseline_generations is None:
                    ...
                    doc_generations = [
                        doc_result.get("generations") for doc_result in seper_results["individual_docs"]
                    ]
                ...
                for i, doc_result in enumerate(individual_doc_results):
                    if i < len(seper_results["individual_docs"]):
                        ...
                        doc_result["utility"]["seper_reduction"] = seper_results["individual_docs"][i][
                            "seper_reduction"
                        ]
            ...
            if semantic_entropy_results:
                ...
                for i, doc_result in enumerate(individual_doc_results):
                    if i < len(semantic_entropy_results["individual_docs"]):
                        doc_result["metrics"]["semantic_entropy"] = semantic_entropy_results["individual_docs"][i][
                            "semantic_entropy"
                        ]
                        doc_result["utility"]["semantic_entropy_reduction"] = semantic_entropy_results[
                            "individual_docs"
                        ][i]["semantic_entropy_reduction"]
    ...
    def evaluate_dataset(self, dataset_path: str, output_path: str, max_samples: Optional[int] = None) -> None:
        ...
        for idx, item in tqdm(enumerate(data), total=len(data), desc="Evaluating"):
            tqdm.write(f"Processing question {idx + 1}/{len(data)}: {item['question'][:50]}...")
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default="data/nq_2024_11_07_12_14_naive/intermediate_data.json")
    parser.add_argument("--model_name", type=str, default="Qwen/Qwen2.5-0.5B-Instruct")
    parser.add_argument(
        "--output_path", type=str, default="data/nq_2024_11_07_12_14_naive/results/seper_results.json"
    )
    parser.add_argument("--api_url", type=str, default="http://localhost:30000/v1/chat/completions")
    ...
    # 通用生成设置
    parser.add_argument("--model_device", type=str, default="cuda:0", help="Device for model calculations")
    parser.add_argument(
        "--num_generations", type=int, default=5, help="Number of generations for metrics calculation"
    )
    parser.add_argument("--generation_temperature", type=float, default=1.0, help="Temperature for generations")
    parser.add_argument("--generation_max_tokens", type=int, default=128, help="Max tokens for generations")
    parser.add_argument(
        "--computation_chunk_size", type=int, default=8, help="Computation chunk size for batched operations"
    )

    # SePer related parameters
    parser.add_argument("--use_seper", action="store_true", help="Use SePer for evaluation")

    # Semantic Entropy related parameters
    parser.add_argument("--use_semantic_entropy", action="store_true", help="Use Semantic Entropy for evaluation")
    parser.add_argument(
        "--strict_entailment", action="store_true", help="Use strict entailment for semantic clustering"
    )

    # Reranker related parameters
    parser.add_argument("--use_reranker", action="store_true", help="Use reranker for evaluation")
    parser.add_argument(
        "--reranker_model",
        type=str,
    ...
    evaluator.evaluate_dataset(
        dataset_path=args.dataset_path, output_path=args.output_path, max_samples=args.max_samples
    )
